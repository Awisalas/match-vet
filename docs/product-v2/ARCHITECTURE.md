...
8. Add Trend Intelligence, Comparable Match Intelligence, Regime Intelligence, and Failure Pattern Intelligence later as separately versioned capabilities.

Start collecting genuine chronological V2 evidence as soon as steps 5 and 6 are working. Preserve that period as the pre-intelligence baseline while architecture cleanup and intelligence work proceed. Evaluate each intelligence capability's predictive contribution on later unseen chronological evidence. Freeze the planned intelligence changes before final V2 revalidation. Build the external app/service layer only after that revalidation. Keep beta, Production Promotion, and subscriptions at their later Product Direction gates.
